"use client";

import { motion } from "motion/react";
import { useEffect, useRef, useState } from "react";
import { twMerge } from "tailwind-merge";

function SantaCapIcon({ className }: Readonly<{ className?: string }>) {
  return (
    <motion.svg
      width="19"
      height="19"
      viewBox="0 0 19 19"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={twMerge("size-4", className)}
      variants={{
        rest: { rotate: 30 },
        hover: {
          rotate: [30, 25, 35, 28, 32, 30],
          transition: { duration: 0.5 },
        },
        focus: {
          rotate: [30, 25, 35, 28, 32, 30],
          transition: { duration: 0.5 },
        },
        tap: {
          rotate: [30, 25, 35, 28, 32, 30],
          transition: { duration: 0.5 },
        },
      }}
    >
      <path d="M17.0894 12.8551C17.0172 13.2886 16.992 14.181 16.8889 14.548C16.8082 14.8355 16.7294 15.0784 16.6409 15.2945C16.2212 16.3188 14.8058 16.2029 13.9539 15.4961C11.8935 13.7866 8.28389 11.3723 5.44925 11.5199C3.60053 11.5199 2.78441 12.2557 1.4747 12.2187C0.462474 12.1901 0.419678 11.1702 0.419678 10.7253C0.419678 9.17739 1.55009 6.94355 2.39598 5.63987C2.67037 5.21699 3.12148 4.94317 3.6252 4.92362C5.66959 4.84428 8.66419 5.47922 10.055 6.05561C11.8624 6.80443 17.4742 5.01432 18.0654 8.17788C18.2019 8.90825 17.2877 11.6655 17.0894 12.8551Z"></path>
      <path d="M14.058 15.0757C14.4647 16.1463 13.7261 17.6881 12.6525 18.096C11.5789 18.5039 9.97218 17.4173 9.56547 16.3467C9.15876 15.2762 9.7717 14.268 10.8453 13.8601C11.9189 13.4522 13.6513 14.0051 14.058 15.0757Z"></path>
      <path
        d="M16.1484 13.3764C16.1484 13.8639 16.1168 14.2139 16.0254 14.5047C15.7048 15.5244 14.2246 15.3306 13.408 14.6408C11.4715 13.0052 8.10014 10.6455 5.47229 10.7758C3.75121 10.7758 2.66599 11.6546 1.44671 11.6218C0.504369 11.5964 0.51242 10.995 0.51242 10.6008C0.51242 9.23692 1.64568 6.94835 2.46272 5.68202C2.72996 5.26782 3.17423 5.00794 3.66678 4.98872C5.59668 4.91345 8.47203 5.42532 9.76002 5.93343C11.8312 6.75022 16.1484 10.557 16.1484 13.3764Z"
        fill="white"
      ></path>
      <path
        d="M15.582 12.1143C15.6921 12.1143 15.8002 12.1226 15.9062 12.1348C16.0599 12.561 16.1484 12.9779 16.1484 13.376C16.1484 13.8635 16.1168 14.2141 16.0254 14.5049C15.7047 15.5244 14.2248 15.3303 13.4082 14.6406C13.3452 14.5874 13.2799 14.5342 13.2139 14.4795C13.1939 14.3736 13.1816 14.265 13.1816 14.1543C13.1819 13.0278 14.2565 12.1143 15.582 12.1143Z"
        fill="#fff"
      ></path>
      <path
        d="M17.9819 8.23983C17.9819 11.3505 17.1426 10.8132 16.0906 12.9176C15.6714 10.8974 15.1672 13.4229 12.5547 15.1421C11.8451 15.609 11.9397 15.5814 11.4871 15.1421C11.4871 13.1202 10.9116 8.10991 10.1557 7.0614C9.27191 5.83531 5.67518 4.99652 3.7019 4.99652C5.38417 2.01401 5.3475 0.159215 6.86264 0.15918C8.88407 0.15918 10.5602 3.64347 13.0577 4.53618C15.9771 5.57968 17.9819 6.56336 17.9819 8.23983Z"
        fill="#D8001B"
      ></path>
      <path
        d="M13.0397 15.0393C13.3801 15.9353 13.0054 17.1366 12.1095 17.477C11.2135 17.8174 10.1356 17.1678 9.79528 16.2719C9.45491 15.376 9.9658 14.533 10.8617 14.1927C11.7577 13.8523 12.6994 14.1434 13.0397 15.0393Z"
        fill="#fff"
      ></path>
      <path d="M17.9771 8.11267C17.9796 8.15455 17.9819 8.19685 17.9819 8.23962C17.9819 11.3503 17.1423 10.813 16.0903 12.9174C15.9222 12.1074 15.7423 12.0298 15.4165 12.3363C16.0977 11.45 17.5889 10.3388 17.9565 8.11169C17.9634 8.1117 17.9702 8.11245 17.9771 8.11267Z"></path>
    </motion.svg>
  );
}

function Box({ parrentWidth }: Readonly<{ parrentWidth: number }>) {
  return (
    <motion.span
      className="bg-amber-300 size-10 rounded-[5px] inline-flex flex-col items-center justify-center gap-0.5 absolute left-0.5 z-10"
      layout
      variants={{
        rest: { rotate: 0, x: 0 },
        hover: { rotate: 360, x: parrentWidth - 48 },
        focus: { rotate: 360, x: parrentWidth - 48 },
        tap: { rotate: 360, x: parrentWidth - 48 },
      }}
      transition={{
        type: "tween",
        duration: 0.2,
      }}
    >
      <span className="grid grid-cols-5 gap-0.5">
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
      </span>

      <span className="grid grid-cols-5 gap-0.5">
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
      </span>

      <span className="grid grid-cols-5 gap-0.5">
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
      </span>

      <span className="grid grid-cols-5 gap-0.5">
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
      </span>

      <span className="grid grid-cols-5 gap-0.5">
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
        <span className="rounded-full size-[3px] bg-white/20"></span>
      </span>
    </motion.span>
  );
}

function OrbitButton({
  children,
  ...props
}: Readonly<{ children: React.ReactNode }>) {
  const buttonRef = useRef<HTMLButtonElement | null>(null);
  const [width, setWidth] = useState(0);

  useEffect(() => {
    if (!buttonRef.current) return;
    setWidth(buttonRef.current.offsetWidth);
  }, [buttonRef, children]);

  return (
    <motion.button
      ref={buttonRef}
      className="relative flex h-12 items-center justify-end cursor-pointer rounded-lg px-4 p-0.5 bg-zinc-900 border-2 border-zinc-700 transition-colors duration-200 ease-in-out hover:bg-zinc-950 focus:bg-zinc-950 focus:outline-none focus:ring-2 focus:ring-amber-300 focus:ring-offset-2 focus:ring-offset-zinc-800"
      initial="rest"
      whileHover="hover"
      whileFocus="focus"
      whileTap="tap"
      animate="rest"
      {...props}
    >
      <Box parrentWidth={width} />

      <motion.span
        variants={{
          rest: { x: 0, marginLeft: 44 },
          hover: { x: -44, marginLeft: 44 },
          focus: { x: -44, marginLeft: 44 },
          tap: { x: -44, marginLeft: 44 },
        }}
      >
        {children}
      </motion.span>

      <SantaCapIcon className="absolute -top-3 -right-3 size-6 z-20 pointer-events-none" />
    </motion.button>
  );
}

export { OrbitButton };
